...
protected:
   ...
   *
   * NOTE: This method is not included in the deal library.
   * In contrast to the modified Gram Schmidt method, this method needs less
   * matrix vector products with M.
   */
  static double
  gram_schmidt(
    const internal::SolverGMRESImplementation::TmpVectors<VectorType>
   ...
   * Calls the signal re_orthogonalize_signal if it is connected.
   *
   * NOTE: This method from the deal.II library was modified to orthogonalize
...
template <class VectorType>
inline double
SolverGMRES<VectorType>::gram_schmidt(
  const internal::SolverGMRESImplementation::TmpVectors<VectorType>
    &                                       orthogonal_vectors,
  const unsigned int                        dim,
  const unsigned int                        accumulated_iterations,
  ...
  const boost::signals2::signal<void(int)> &reorthogonalize_signal)
{
  ...
  if (consider_reorthogonalize)
    norm_vv_start = std::sqrt(tmp*vv); //vv.l2_norm();

  // Orthogonalization
  M.vmult(tmp,vv);
  h(0) = tmp*orthogonal_vectors[0]; //vv * orthogonal_vectors[0];
  for (unsigned int i = 0; i < dim; ++i)
  {
	h(i) = tmp*orthogonal_vectors[i];
  }
  for (unsigned int i = 0; i < dim; ++i)
  {
  ...
  M.vmult(tmp,vv);
  double norm_vv = std::sqrt(tmp*vv);
  ...
  //Here, if reorthogonalization is activated, the modified Gram Schmidt is used
  if (reorthogonalize == true)
    {
      double htmp = M.matrix_scalar_product(vv,orthogonal_vectors[0]); //vv * orthogonal_vectors[0];
      h(0) += htmp;
      for (unsigned int i = 1; i < dim; ++i)
        {
    	  vv.add(-htmp,orthogonal_vectors[i - 1]);
    	  htmp = M.matrix_scalar_product(vv,orthogonal_vectors[i]);
          //htmp = vv.add_and_dot(-htmp,
          //                      orthogonal_vectors[i - 1],
          //                      orthogonal_vectors[i]);
          h(i) += htmp;
        }
      vv.add(-htmp,orthogonal_vectors[dim - 1]);
      norm_vv =  std::sqrt(M.matrix_norm_square(vv));
  ...
  const boost::signals2::signal<void(int)> &reorthogonalize_signal)
{
  ...
  const bool consider_reorthogonalize =
    (reorthogonalize == false) && (inner_iteration % 5 == 4);
  if (consider_reorthogonalize)
    norm_vv_start = std::sqrt(M.matrix_norm_square(vv)); //vv.l2_norm();

  // Orthogonalization
  h(0) = M.matrix_scalar_product(vv,orthogonal_vectors[0]); //vv * orthogonal_vectors[0];
  for (unsigned int i = 1; i < dim; ++i)
  {
	vv.add(-h(i - 1),orthogonal_vectors[i - 1]);
	h(i) = M.matrix_scalar_product(vv,orthogonal_vectors[i]);
    //h(i) = vv.add_and_dot(-h(i - 1),
    //                      orthogonal_vectors[i - 1],
    //                      orthogonal_vectors[i]);
  }
  vv.add(-h(dim - 1),orthogonal_vectors[dim - 1]);

  double norm_vv = std::sqrt(M.matrix_norm_square(vv));
    //std::sqrt(vv.add_and_dot(-h(dim - 1), orthogonal_vectors[dim - 1], vv));
  ...
  do
    {
      ...
      double rho =  std::sqrt(M.matrix_norm_square(v));//v.l2_norm();
