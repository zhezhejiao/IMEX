...
template<int dim>
vIMEX<dim>::vIMEX(const SparseMatrix<double> &M,
...
template<int dim>
void vIMEX<dim>::update_matrices(const double time) {
...
template <int dim>
void vIMEX<dim>::integrate_step(Vector<double>& u,
					Vector<double>& Mv,
					const double time)
{
	double h1=pow(time+stepsize+1,-1);
	...
	system_rhs += Mv;
	scaled_Mv = Mv;
    scaled_Mv *= (stepsize / 4.0)*(h1 - h);
    system_rhs += scaled_Mv;
...
template class vIMEX<3>;
template class vIMEX<2>;
